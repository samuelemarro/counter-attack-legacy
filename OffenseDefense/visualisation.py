...
def plot_histogram(values, color, log_x=False):
    ...
    if sorted_values.shape[0] > 1:
        ...
        kde = KernelDensity(kernel='gaussian', bandwidth=0.2).fit(
            sorted_values[:, np.newaxis].reshape(-1, 1))
...
def plot_roc(genuine_values, adversarial_values):
    ...
    plt.plot(false_positive_rates, true_positive_rates, color='darkorange',
             lw=lw, label='ROC curve (area = {:.2f}%)'.format(area_under_curve * 100.0))
    plt.plot([0, 1], [0, 1], color='navy', lw=lw, linestyle='--')
    plt.plot([best_fpr, best_fpr], [0, best_tpr],
             color='green', lw=lw, linestyle='--')
    plt.xlim([0.0, 1.0])
    plt.ylim([0.0, 1.0])
    plt.xlabel('False Positive Rate (Best = {:2.2f}%)'.format(
        best_fpr * 100.0))
